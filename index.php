...
<html lang="en">
  ...
  <body>
	...
	<div class="col-6 mx-auto">
		<a href="admin.php" class="text-center">Добавить пост</a>
				...
				<?php 
							...
							<div class="mt-5 border rounded">
								<form action="delete.php" method="GET">
									<input type="" name="id" value="<?php echo $stroka["id"];?>" style="display:none;">
										...
										<button class="btn text-light bg-danger">удалить</button>
					...
					<div class="col-12 px-1">
						...
						<form action="update.php" method="GET">
							<div class="izmen" style="display:none">
							<input type="" name="id" value="<?php echo $stroka["id"];?>" style="display:none;">
							<input type="file" name="img" class="vvod">
							<button class=" btn bg-primary text-light redach1" >редачить</button>
						</div>
						</form>
					...
					<div class="col-12 py-2">  <!--текстовые элементы поста-->
						...
						<div>
							<p class=tekst><?php echo $stroka["text"]; ?></p>
							<div class="row neww" style="display:none">
								<form action="update.php" method="GET">
									<input type="" name="id" value="<?php echo $stroka["id"];?>" style="display:none;">
									<input type="" name="text" class="vvod" value="<?php echo $stroka["text"]; ?>">
								<button class=" btn bg-primary text-light redach" >редачить</button>
								</form>
	...
	<script type="text/javascript">
		let tekst = document.querySelectorAll(".tekst");
		let vvod = document.querySelectorAll(".vvod");
		let redach = document.querySelectorAll(".redach");
		let redach1 = document.querySelectorAll(".redach1");
		let neww = document.querySelectorAll(".neww");
		let kartinka = document.querySelectorAll(".kartinka");
		let izmen = document.querySelectorAll(".izmen");

		for(let i = 0; i < 4; i++){

			tekst[i].onclick = function(){
			
			neww[i].style.display = "block";
			tekst[i].style.display = "none";

		}
		redach[i].onclick = function(){
			neww[i].style.display = "none";
			tekst[i].style.display = "block";
		}

		kartinka[i].onclick = function(){
				izmen[i].style.display = "block";


		}
		redach[i].onclick=function(){
			izmen[i].style.display= "none";
		}
